 ...
 *
 * This sample is taken from: https://github.com/mangeshpardhi/procedural_devkit
 *
...
hbm_create(
    ...
    array(
        'description'=>'Billplz Module for HostBill',
        'version'=>'3.0',
        'currencies'=>array('MYR', 'RM') //list of supported currencies
    )
);

hbm_add_config_option(
    'API Secret Key',
    array(
     'type'=>'input',
     'default'=>'ed586547-00b7-459a-a02e-7e876a744590',
     'description'=>'Please enter your Billplz API Secret Key. Get the key on Billplz >> Account Settings.',
    )
);
...
hbm_add_config_option(
    ...
    array(
     'type'=>'input',
     'default'=>'S-0Sq67GFD9Y5iXmi5iXMKsA',
     'description'=>'Please enter your Billplz X Signature Key. Get the key on Billplz >> Account Settings.',
    )
);
...
hbm_add_config_option(
    ...
    array(
     'type'=>'input',
     'default'=>'ugo_7dit',
     'description'=>'Leave blank if you are unsure.',
    )
);

hbm_on_action('payment.displayform', function ($details) {
    $hostbill_details = hbm_get_hostbill_details();
    $x_signature = hbm_get_config_option('X Signature Key');

    //This will create page for Billplz to create bill route
    $create_bill = hbm_client_url('billplz_create_bill');

    $amount = $details['invoice']['amount'];
    $item_number = $details['invoice']['id'];
    $client_id = $details['client']['client_id'];

    $raw_string = $amount .$item_number . $client_id;
    $filtered_string = preg_replace("/[^a-zA-Z0-9]+/", "", $raw_string);
    $hash = hash_hmac('sha256', $filtered_string, $x_signature);

    $form =  "<form action='{$create_bill}' method='post' name='billplz_payform'>";
    $form .=  '<input type="hidden" value="utf-8" name="charset">
        ...
        <input type="hidden" value="'.$details['client']['firstname'].' '.$details['client']['lastname'].'" name="name">
        <input type="hidden" value="'.$details['client']['email'].'" name="email">
        <input type="hidden" value="'.$details['client']['mobile'].'" name="mobile">
        <input type="hidden" value="'.$details['client']['client_id'].'" name="client_id">
        <input type="hidden" value="'.$hash.'" name="hash">
        <input type="submit" value="Pay Now" >';

    $form .="</form>";
    $form .='<script>document.billplz_payform.submit();</script>';
    return $form;
});

// Create route that Billplz will create the bill
hbm_client_route('billplz_create_bill', function ($params) {

    //This will create url to callback route created below
    $callback_url = hbm_client_url('billplz_callback');

    $api_key=hbm_get_config_option('API Secret Key');
    $collection_id = hbm_get_config_option('Collection ID');
    $x_signature = hbm_get_config_option('X Signature Key');

    $amount = isset($_POST['amount']) ? $_POST['amount'] : die('Amount parameter is not passed');
    $item_number = isset($_POST['item_number']) ? $_POST['item_number'] : die('Item number parameter is not passed');
    $client_id = isset($_POST['client_id']) ? $_POST['client_id'] : die('Client ID parameter is not passed');
    $description = isset($_POST['item_name']) ? $_POST['item_name'] : die('Item name parameter is not passed');
    ...
    $billplz
        ...
        ->setReference_1($item_number.'|'.$client_id)
...
// Create route that Billplz can response to Redirect and Callback

hbm_client_route('billplz_callback', function ($params) {
    $api_key=hbm_get_config_option('API Secret Key');
    $x_signature = hbm_get_config_option('X Signature Key');
    ...
    if ($data['paid']) {
        $item_number_client_id =  explode("|", $moreData['reference_1']);

        $api = new ApiWrapper();
        $params = array(
          'id'=>$item_number_client_id[0]
        );
        $return = $api->getInvoiceDetails($params);
        echo '<h1>Payment Success!</h1>';

        $flag_dont_update = false;
        if ($return['invoice']['status'] === 'Paid') {
            foreach ($return['transactions'] as $transact) {
                if ($transact['trans_id'] === $data['id']) {
        ...
        if (!$flag_dont_update) {
            hbm_add_transaction($item_number_client_id[0], number_format($moreData['amount']/100, 2), array(
                'description' => $mroeData['description'],
                'fee' => '0',
                'transaction_id' => $data['id']
            ));
            hbm_log_callback($data, $results);
        }
        return true;
    }
    echo '<h1>Payment Failed!</h1> <a href="'.$moreData['url'].'">Click here</a> to pay';
    return false;
});
